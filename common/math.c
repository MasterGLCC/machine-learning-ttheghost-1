...
void table_print_head(const Table *t, uint n, const char *name) {
    ...
    if (t->rows > show) printf("  ... (%d more rows)\n", t->rows - show);
    printf("\n");
}
...
Table table_extract_rows(const Table *t, uint row_start, uint row_end) {
    ...
    memcpy(sub.data, t->data + row_start * t->cols, (size_t)new_rows * t->cols * sizeof(f32));
    return sub;
}
