...
// fonction sigmoide
// σ(z) = 1 / (1 + e^(-z))
static inline f32 sigmoid(f32 z) {
...
// Calcul du gradient de la perte par rapport à θ
...
Table gradient_descent(const Table *X, const Table *y, f32 alpha, uint nb_iterations) {
    uint d = X->cols;
    Table theta = init_table(d, 1);

    // Initialisation des parametres à zero
    for (uint i = 0; i < d; i++) {
    ...
    for (uint iter = 0; iter < nb_iterations; iter++) {
        ...
        // Mise à jour : θ = θ - alpha * ∇J(θ)
        for (uint i = 0; i < d; i++) {
            f32 new_val = table_get(&theta, i, 0) - alpha * table_get(&grad, i, 0);
            table_set(&theta, i, 0, new_val);
        }

        // Affichage de la perte toutes les 100 iterations (ou à la fin)
...
int main() {
  printf("=== Regression Logistique avec Descente de Gradient ===\n\n");
    ...
    for (uint i = 0; i < n; i++) {
        printf("x = (%.0f, %.0f) -> y = %.0f\n",
               table_get(&X, i, 1), table_get(&X, i, 2), table_get(&y, i, 0));
    }
    printf("\n");
