...
int main() {
  ...
  // Converte les paramètres du modèle normalisé à l'échelle d'origine
  // Pour a on utilise la formule: a_orig = (stddev_y / stddev_x) * a_norm
  // Pour b on utilise la formule: b_orig = mean_y - a_orig * mean + stddev_y * b_norm
  f32 a_orig = (stddev_y / stddev_x) * model.a;
  f32 b_orig = mean_y - a_orig * mean_x + stddev_y * model.b;

  printf("Normalized: a = %f, b = %f\n", model.a, model.b);
  printf("Original scale: a = %f, b = %f\n", a_orig, b_orig);
